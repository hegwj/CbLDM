...
class Encoder(nn.Module):
    def __init__(self, *, ch=64, ch_mult=(1, 2, 4, 4), num_res_blocks=2,
                 attn_resolutions=(1, 2, 8), resamp_with_conv=True, in_channels=4,
                 resolution=128, z_channels=1, double_z=True, pdf_len=3000, pdf_ch=512, **ignore_kwargs):
        super().__init__()
        self.ch = ch
        self.temb_ch = 0
        ...
        self.in_channels = in_channels
        self.posterior_size = resolution // 2 ** len(attn_resolutions)

        self.actfun = nn.SiLU()
        self.prior_1 = torch.nn.Linear(pdf_len, pdf_ch)
        self.prior_2 = torch.nn.Linear(pdf_ch, pdf_ch)
        self.prior_3 = torch.nn.Linear(pdf_ch, z_channels * self.posterior_size ** 2)
    ...
    def forward(self, x, pdf):
        ...
        pdf = torch.nn.functional.interpolate(pdf_prior, size=self.resolution)
        ...
        pdf_prior = torch.cat((pdf_prior, pdf_prior), dim=1)
...
class Decoder(nn.Module):
    def __init__(self, *, ch=64, out_ch=4, ch_mult=(1, 2, 4, 4), num_res_blocks=2,
                 attn_resolutions=(1, 2, 8), resamp_with_conv=True, in_channels=4,
